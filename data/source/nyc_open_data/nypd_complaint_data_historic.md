...
| Portal CSV export endpoint | <https://data.cityofnewyork.us/api/views/qgea-i56i/rows.csv?accessType=DOWNLOAD> |
...
The raw CSV is intentionally ignored by Git and is never sent to the
...
To retrieve the portal export available at retrieval time from the repository
root:
...
This command retrieves the portal snapshot available at invocation; it is not
expected to recreate the reviewed historical bytes after the portal data
changes.
...
timestamp. The reviewed snapshot uses `--as-of-date 2026-07-04`; pass that
